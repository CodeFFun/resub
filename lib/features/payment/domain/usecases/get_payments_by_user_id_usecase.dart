...
final getPaymentsByUserIdUsecaseProvider = Provider<GetPaymentsByUserIdUsecase>(
  (ref) {
    final paymentRepository = ref.read(paymentRepositoryProvider);
    return GetPaymentsByUserIdUsecase(paymentRepository: paymentRepository);
  },
);

class GetPaymentsByUserIdUsecase
    implements UsecaseWithoutParms<List<PaymentEntity>> {
  final IPaymentRepository _paymentRepository;

  GetPaymentsByUserIdUsecase({required IPaymentRepository paymentRepository})
    : _paymentRepository = paymentRepository;

  @override
  Future<Either<Failure, List<PaymentEntity>>> call() {
    return _paymentRepository.getPaymentsByUserId();
  }
}
