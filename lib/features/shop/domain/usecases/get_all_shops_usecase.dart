...
final getAllShopsUsecaseProvider = Provider<GetAllShopsUsecase>((ref) {
  final shopRepository = ref.read(shopRepositoryProvider);
  return GetAllShopsUsecase(shopRepository: shopRepository);
});

class GetAllShopsUsecase implements UsecaseWithoutParms<List<ShopEntity>> {
  final IShopRepository _shopRepository;

  GetAllShopsUsecase({required IShopRepository shopRepository})
